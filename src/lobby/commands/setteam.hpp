...
bool SetTeamCommand::execute(nnwcli::CommandExecutorContext* const ctx, void* const data)
{
    ...
    *parser >> playername;
    ...
    // Support existing red/blue/none and Hide & Seek aliases
    std::string tn = teamname;
    std::transform(tn.begin(), tn.end(), tn.begin(), ::tolower);
    if (tn == "blue" || tn == "b" || tn == "seek" || tn == "seeker" || tn == "s")
        team = KART_TEAM_BLUE;
    else if (tn == "red" || tn == "r" || tn == "hide" || tn == "hider" || tn == "h")
        team = KART_TEAM_RED;
    else if (tn == "spectator" || tn == "spec" || tn == "none" || tn == "n")
        team = KART_TEAM_NONE;
